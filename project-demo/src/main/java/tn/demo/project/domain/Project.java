...
public class Project implements Persistable<UUID> {
    ...
    public static Project createNew(ProjectId projectId, String name, String description, LocalDateTime now, LocalDate plannedEndDate, TimeEstimation timeEstimation, ContactPersonId contactPersonId, String contactPersonName, String email) {
        Set<ContactPerson> contactPersons = new HashSet<>();
        contactPersons.add(ContactPerson.createNew(contactPersonId, contactPersonName, email));
        return new Project(projectId.value(), name, description, now, plannedEndDate, ProjectStatus.PLANNED, 0, true, new HashSet<>(), timeEstimation, contactPersons);
    }
    ...
    private Project(UUID id, String name, String description,
                    ...
                    TimeEstimation originalEstimatedTime, Set<ContactPerson> contactPerson) {
        this.id = id;
        this.name = name;
    ...
    public Project completeTask(ProjectTaskId projectTaskId, ActualSpentTime actualSpentTime) {
        ...
        var newStatus = areAllTasksCompleted(processedTasks)
                ...
                : status;
        return new Project(id, name, this.description, createdAt, plannedEndDate, newStatus, version, false, processedTasks, getInitialEstimation(), contactPerson);
    }
    ...
    public String getContactPersonEmail(){
        return contactPerson.iterator().next().getEmail();
    }
